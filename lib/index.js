...
function extractModelIds(body) {
	const rows = Array.isArray(body) ? body : isRecord$4(body) && Array.isArray(body["data"]) ? body["data"] : isRecord$4(body) && Array.isArray(body["models"]) ? body["models"] : [];
	const ids = [];
	for (const row of rows) if (typeof row === "string" && row.length > 0) ids.push(row);
	else if (isRecord$4(row) && typeof row["id"] === "string" && row["id"].length > 0) ids.push(row["id"]);
	return [...new Set(ids)];
}
...
async function fetchLiveModelIds(accessToken, signal) {
	...
	if (!response.ok) {
		const code = isRecord$4(body) && typeof body["error"] === "string" ? body["error"] : void 0;
...
function isGrokAuthDocument(value) {
	if (!isRecord$3(value) || "version" in value && "credential" in value) return false;
...
function writeGrokAuthDocument(existingText, credential) {
	...
	if (existingText !== void 0 && existingText.trim().length > 0) {
		...
		if (!isRecord$3(parsed)) throw new Error("xai-oauth: Grok CLI auth file must contain an object");
		document = { ...parsed };
	}
...
const USER_AGENT$1 = "dsh-grok-kit/0.1.4";
...
function wrapXaiResponsesProvider(provider, options) {
	const run = (fn, model, context, streamOptions) => {
		...
		if (!options.retry401 || options.tokenSource === void 0) return decorate(begin(streamOptions?.apiKey));
		const rejected = streamOptions?.apiKey;
		if (rejected === void 0 || rejected.length === 0) return decorate(begin(streamOptions?.apiKey));
