...
function sanitizeStoredOptions(value) {
	...
	if (maxResults !== void 0 && maxResults <= SEARCH_MAX_RESULTS_CEILING) out["searchMaxResults"] = Math.trunc(maxResults);
	const webTimeout = positiveFinite(raw["webSearchTimeoutMs"]);
	if (webTimeout !== void 0 && webTimeout <= TIMEOUT_CEILING_MS) out["webSearchTimeoutMs"] = Math.trunc(webTimeout);
	const xTimeout = positiveFinite(raw["xSearchTimeoutMs"]);
	if (xTimeout !== void 0 && xTimeout <= TIMEOUT_CEILING_MS) out["xSearchTimeoutMs"] = Math.trunc(xTimeout);
	return out;
}
...
const USER_AGENT$1 = "dsh-grok-kit/0.1.6";
...
const USER_AGENT = "dsh-grok-kit/0.1.6";
