...
function sanitizeStoredOptions(value) {
	...
	if (typeof raw["searchModel"] === "string" && raw["searchModel"].trim().length > 0) out["searchModel"] = raw["searchModel"].trim();
	const maxResults = positiveFinite(raw["searchMaxResults"]);
	if (maxResults !== void 0) out["searchMaxResults"] = Math.trunc(maxResults);
	const webTimeout = positiveFinite(raw["webSearchTimeoutMs"]);
	if (webTimeout !== void 0) out["webSearchTimeoutMs"] = Math.trunc(webTimeout);
	const xTimeout = positiveFinite(raw["xSearchTimeoutMs"]);
	if (xTimeout !== void 0) out["xSearchTimeoutMs"] = Math.trunc(xTimeout);
	return out;
}
...
const USER_AGENT$1 = "dsh-grok-kit/0.1.5";
