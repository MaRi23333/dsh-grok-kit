...
function parseLockPid(text) {
	...
	if (!/^\d{1,10}$/.test(first)) return void 0;
...
*/
async function breakStaleEmptyLock(lockPath) {
	const second = await readLockText(lockPath);
	if (second === void 0 || second.trim().length > 0) return void 0;
	let mtimeMs;
	try {
	...
	if (Number.isNaN(mtimeMs) || Date.now() - mtimeMs < 1e4) return void 0;
	try {
...
* follows it.
*/
async function breakStaleWriterLock(lockPath) {
	const first = await readLockText(lockPath);
	if (first === void 0) return void 0;
	const pid = parseLockPid(first);
	if (pid === void 0) return breakStaleEmptyLock(lockPath);
	if (isPidAlive(pid) !== false) return void 0;
	if (await readLockText(lockPath) !== first) return void 0;
	try {
		await rm(lockPath, { force: true });
	} catch {
		return;
	...
	*/
	async clearStaleLock() {
		try {
			...
			if (pid === 0) console.warn(`dsh-grok-kit: removed stale empty writer lock ${this.lockFilename}.lock (no pid recorded; owner died between create and write); continuing normally.`);
			else if (pid !== void 0) console.warn(`dsh-grok-kit: removed writer lock ${this.lockFilename}.lock left by dead process ${pid}; continuing normally.`);
		} catch {}
	}
...
function sanitizeStoredOptions(value) {
	if (!isRecord$3(value) || value["version"] !== OPTIONS_VERSION) return {};
	const raw = isRecord$3(value["options"]) ? value["options"] : {};
	const out = {};
	for (const key of [
		"backendSearch",
		"nestedSearchTools",
	...
	ctx.effect(() => {
		const routes = [
			...
			ctx.webServer.register({
				...
				handler: async (req, res) => {
					if (!trustedRequest(req)) return json(res, 403, { error: "forbidden" });
					if (req.method === "GET") return json(res, 200, { options: readStoredOptions() });
					if (req.method !== "POST") return json(res, 405, { error: "method not allowed" });
					try {
						const body = await readJson(req);
...
function collectOutputText(body) {
	...
	for (const item of outputItems(body)) {
		if (!isRecord$2(item) || item["type"] !== "message" || !Array.isArray(item["content"])) continue;
		for (const chunk of item["content"]) {
			if (!isRecord$2(chunk) || chunk["type"] !== "output_text") continue;
...
function collectActionSources(body) {
	const sources = [];
	for (const item of outputItems(body)) {
		if (!isRecord$2(item) || !isSearchCall(item) || !isRecord$2(item["action"])) continue;
...
function collectAnnotationSources(body) {
	...
	for (const item of outputItems(body)) {
		if (!isRecord$2(item) || item["type"] !== "message" || !Array.isArray(item["content"])) continue;
		for (const chunk of item["content"]) {
			if (!isRecord$2(chunk) || chunk["type"] !== "output_text" || !Array.isArray(chunk["annotations"])) continue;
			for (const annotation of chunk["annotations"]) {
				if (!isRecord$2(annotation)) continue;
...
function collectCustomToolSources(body) {
	const sources = [];
	for (const item of outputItems(body)) {
		if (!isRecord$2(item) || item["type"] !== "custom_tool_call" || !isXSearchToolName(item["name"])) continue;
		for (const key of [
			"output",
			"result",
			"content"
...
function collectToolUsage(body) {
	...
	if (!isRecord$2(usage)) return void 0;
	const details = usage["server_side_tool_usage_details"];
	if (!isRecord$2(details)) return void 0;
	const counts = {};
...
function isAbortError(error) {
	...
	return isRecord$2(error) && error["name"] === "AbortError";
}
...
var XaiOAuthSearchProvider = class {
	...
	async search(request, signal) {
		...
		if (!isRecord$2(body)) throw new XaiOAuthSearchError("xAI search returned an invalid response envelope", "WEB_PROVIDER_ERROR");
...
function parseDocument(text) {
	...
	if (!isRecord$1(value) || value["version"] !== CHAINS_VERSION || !isRecord$1(value["sessions"])) return {};
	const sessions = value["sessions"];
	const out = {};
	for (const [sessionId, raw] of Object.entries(sessions)) {
		if (!isRecord$1(raw) || typeof raw["responseId"] !== "string" || typeof raw["model"] !== "string") continue;
