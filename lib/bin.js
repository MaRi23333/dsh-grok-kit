...
function isGrokAuthDocument(value) {
	if (!isRecord$1(value) || "version" in value && "credential" in value) return false;
...
function slotRecord(document) {
	if (isRecord$1(document[GROK_XAI_SLOT_KEY])) return {
...
function writeGrokAuthDocument(existingText, credential) {
	...
	if (existingText !== void 0 && existingText.trim().length > 0) {
		...
		if (!isRecord$1(parsed)) throw new Error("xai-oauth: Grok CLI auth file must contain an object");
		document = { ...parsed };
	}
