...
function XaiSettings({ t }) {
	...
	const loadOptions = (0, react.useCallback)(async () => {
		try {
			const value = await jsonRequest(OPTIONS_PATH);
	...
	const saveOptions = async () => {
		...
		try {
			...
			const value = await jsonRequest(OPTIONS_PATH, "POST", patch);
			setOptions(value.options ?? {});
			setOptionsDirty(/* @__PURE__ */ new Set());
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("nestedSearchTools") }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
							value: options.nestedSearchTools === void 0 ? "auto" : String(options.nestedSearchTools),
							disabled: optionsBusy,
...
const en = {
	...
	backendSearchHint: "Mixes xAI server-side web/X search into the grok-4.6 main request (thinking IS the search). Turning it off falls back to the nested-search path.",
...
const zh = {
	...
	backendSearchHint: "在 grok-4.6 主请求混合 xAI 服务端 web/X 搜索（THINK 即搜索）。关闭后回退到嵌套搜索路径。",
