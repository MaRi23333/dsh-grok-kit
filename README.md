...
- **Windows ACLs:** Node's file mode bits are not Windows ACLs. Credential protection on Windows relies on the `$DSH_HOME` directory being user-private (true for `%USERPROFILE%\.dsh`); if you relocate `DSH_HOME` to a shared location, restrict its ACL yourself.
